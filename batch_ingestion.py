...
def delta_append(table_name, data, unique_key):
    ...
    print(f"Appending data to Snowflake table: {table_name}...")
...
# Append to Snowflake table every 100 blocks
def fetch_and_store_transactions(start_block, end_block, batch_size=100):
    data = []
    ...
    for block_num in range(start_block, end_block + 1):
        ...
        if (block_num - start_block + 1) % batch_size == 0:
            end_time = time.time()
            print(f"Time taken to fetch {batch_size} blocks: {end_time - start_time:.2f} seconds")
            delta_append('TRANSACTIONS', data, 'TX_HASH')
            start_time = time.time()
            data = []

    # Delta append to Snowflake
    if data:
        delta_append('TRANSACTIONS', data, 'TX_HASH')

# Function to fetch and store raw logs
def fetch_and_store_logs(start_block, end_block):
    data = []
    for block_num in range(start_block, end_block + 1):
        block = web3.eth.get_block(block_num, full_transactions=True)
        block_time = block['timestamp']
...
def fetch_and_store_traces(start_block, end_block):
    data = []
    for block_num in range(start_block, end_block + 1):
        block = web3.eth.get_block(block_num, full_transactions=True)
        for tx in block['transactions']:
            print('transaction: '+str(tx.hash.hex()))
            # trace_data = fetch_trace_data(tx['hash'])
...
def main():
    latest_block = web3.eth.block_number #28382323 #28573438
    ...
    fetch_and_store_transactions(start_block_tx, latest_block)
    fetch_and_store_logs(start_block_log, latest_block)
    fetch_and_store_traces(start_block_trace, latest_block)
