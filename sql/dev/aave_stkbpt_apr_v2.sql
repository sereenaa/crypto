select * from tokenlogic-data.events.aave_stk_token_transfer order by block_timestamp desc limit 100;
select * from tokenlogic-data.events.aave_stk_token_transfer where stake_token = '0x9eda81c21c273a82be9bbc19b6a6182212068101' order by block_timestamp desc;

select timestamp_seconds(block_timestamp) as blck_timestamp, *
from tokenlogic-data.indexer_prod.stk_token_transfer where contract_address = '0x9eda81c21c273a82be9bbc19b6a6182212068101' order by block_timestamp desc;

select timestamp_seconds(block_timestamp) as blck_timestamp, *
from tokenlogic-data-dev.indexer_prod.stk_token_transfer where contract_address = '0x9eda81c21c273a82be9bbc19b6a6182212068101' order by block_timestamp desc;

select max(timestamp_seconds(block_timestamp)) from tokenlogic-data-dev.indexer_prod.stk_token_transfer;
select max(timestamp_seconds(block_timestamp)) from tokenlogic-data.indexer_prod.stk_token_transfer;

-- Insert rows from the dev table into the main table
-- INSERT INTO tokenlogic-data.indexer_prod.stk_token_transfer
-- SELECT *
-- FROM tokenlogic-data-dev.indexer_prod.stk_token_transfer
-- WHERE block_timestamp > (
--   SELECT max(block_timestamp)
--   FROM tokenlogic-data.indexer_prod.stk_token_transfer
-- );
